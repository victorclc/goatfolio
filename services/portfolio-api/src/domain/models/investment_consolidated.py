...
class StockConsolidated(InvestmentConsolidated):
    ...
    class StockPositionWrapperLinkedList:
        class StockPositionWrapper:
            ...
            def amount(self):
                if self.prev:
                    return (
                        self.prev.amount
                        + self.data.bought_amount
                        - self.data.sold_amount
                    )
            ...
            def current_invested_value(self):
                if self.prev:
                    return (
                        self.bought_value - self.current_invested_sold_value
                    ).quantize(Decimal("0.01"))
                return (
                    self.data.bought_value - self.current_invested_sold_value
                ).quantize(Decimal("0.01"))

            @property
            def node_invested_value(self):
                return (
                    self.data.bought_value - self.data.sold_amount * self.average_price
                )
            ...
            def month_variation_percent(self):
                if self.prev and self.prev.amount > 0:
                    prev_gross_value = (
                        self.prev_adjusted_gross_value + self.data.bought_value
                    )
                else:
        ...
        def push(self, new_val):
            ...
            new_node = self.StockPositionWrapper(new_val)
    ...
    def monthly_stock_position_wrapper_linked_list(
        ...
        doubly = self.StockPositionWrapperLinkedList()
