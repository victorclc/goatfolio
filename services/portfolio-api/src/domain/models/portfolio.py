...
class StockPositionMonthlySummary:
    ...
    def __post_init__(self):
        if type(self.date) is not datetime:
            self.date = datetime.fromtimestamp(self.date, tz=timezone.utc)  # type: ignore
...
class StockSummary:
    ...
    previous_position: Optional[StockPositionMonthlySummary] = None
    alias_ticker: str = ""

    def __post_init__(self):
        if type(self.latest_position) is not StockPositionMonthlySummary:
            self.latest_position = StockPositionMonthlySummary(**self.latest_position)  # type: ignore
        if (
            self.previous_position
            and type(self.previous_position) is not StockPositionMonthlySummary
        ):
            self.previous_position = StockPositionMonthlySummary(
                **self.previous_position  # type: ignore
            )
...
class Portfolio:
    ...
    initial_date: datetime = datetime(
        datetime.max.year, datetime.max.month, datetime.max.day, tzinfo=timezone.utc
    )
    stocks: List[StockSummary] = field(default_factory=list)  # todo list type hint

    def __post_init__(self):
        if not isinstance(self.initial_date, datetime):
            self.initial_date = datetime.fromtimestamp(
                float(self.initial_date), tz=timezone.utc  # type: ignore
            )
        self.stocks = [
            StockSummary(**s) if not isinstance(s, StockSummary) else s  # type: ignore
    ...
    def to_dict(self):
        return {
            **self.__dict__,
            "initial_date": int(self.initial_date.timestamp()),
...
class StockConsolidated:
    ...
    def __post_init__(self):
        ...
        self.history = [
            StockPosition(**h) if not isinstance(h, StockPosition) else h
